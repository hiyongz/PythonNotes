测试开发笔记
========================================

测试开发相关笔记，包括Python、Golang、测试工具、自动化测试、数据库、Linux、持续集成等。

.. toctree::
   :maxdepth: 2
   :caption: Python基础知识
   
   python/Python-Enhancement-Proposals.md
   python/python-vscode-jupyter-notebook.md
   python/python-notes-for-list.md
   python/python-notes-for-string.md
   python/python-notes-for-string-interning.md
   python/python-notes-for-variable-and-attribute.md
   python/python-notes-for-datetime.md
   python/python-library-for-click.md
   python/python-notes-for-getopt.md
   python/python-notes-for-passing-arguments-in-function.md
   python/python-notes-for-new-and-init.md
   python/python-notes-for-object-and-copy.md
   

.. toctree::
   :maxdepth: 2   
   :caption: Python进阶
   
   python/python-notes-for-anonymous-function.md
   python/python-notes-for-decorator.md
   python/python-notes-for-function-closures.md
   python/python-notes-for-reflection.md
   python/python-notes-for-multi-process.md
   python/python-notes-for-coroutines.md
   python/python-library-for-json.md
   python/python-library-for-pyyaml.md
   python/python-scapy-for-packet-build-and-parser.md
