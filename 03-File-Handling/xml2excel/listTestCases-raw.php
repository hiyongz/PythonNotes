...
if ($gui->childs && $do_export) {
  $project_name = null;
  $model_node = current($gui->childs);
  $model_id = intval($model_node['id']);
  ...
  $fpath = $basepath . $user . "/testcases/";
  if (is_null($project_name)) {
    $zipfilename = $basepath . $user . "/result.zip";
  } else {
    $zipfilename = $basepath . $user . "/" . str_replace("*", '', $project_name) . ".zip";
  }
  // $zipfilename=iconv("utf-8","gb2312//IGNORE",$zipfilename);
  $imgpath = $basepath . $user . "/images/";
  deldir($imgpath);
  deldir($fpath);
  ...
  // var_dump(array_keys($gui->childs));
  foreach ($gui->childs as $node) {
    $model_name = $node['name'];
    $model_id = intval($node['id']);
    ...
    if (array_key_exists('childNodes', $node)) {
      foreach ($node['childNodes'] as $child_node) {
        $submodel_name = $child_node['name'];
        $submodel_id = intval($child_node['id']);
        $summodel_index = substr($submodel_name, 0, strpos($submodel_name, ']'));
            ...
            foreach ($strs as $str) {
              ...
              switch ($info_array[0]) {
                ...
                case 'img':
                  ...
                  $filename = $imgpath . $case_id . $topoindex . ".jpg";
                  file_put_contents($filename, base64_decode($pic_info['img']));
                ...
                foreach ($item_node['childNodes'] as $case_node) {
                  ...
                  if ($case_id == $sum_id) {
                    ...
                    foreach ($titles as $title) {
                      $case_sheep->setCellValue(chr($begincol + $index_col) . $auto_index, $title);
                      $case_sheep->getColumnDimension(chr($begincol + $index_col))->setWidth(15);
                      $index_col += 1;
                    }
                    $auto_index += 1;
                    $dotitle = true;
        ...
        if (is_null($objPHPExcel) && !$bauto) {
          //设定表头
          $objPHPExcel = new PHPExcel();
          $objPHPExcel->setActiveSheetIndex(0);
          $objPHPExcel->getDefaultStyle()->getFont()->setName('宋体');
          $objPHPExcel->getDefaultStyle()->getFont()->setSize(9);
          $this_sheep = $objPHPExcel->getActiveSheet();
          ...
          $model_prefer = substr($model_name, 0, strpos($model_name, ']'));
          $index += 1;
        }
        if (!$bauto) {
          $new_submodel_name = str_replace($model_prefer . ".", '', $submodel_name);
          $submodel_prefer = substr($submodel_name, 0, strpos($submodel_name, ']'));
          $MainWorkSheet->setCellValue('B' . $index, $new_submodel_name);
          $index += 1;
          if (array_key_exists('childNodes', $child_node)) {
            foreach ($child_node['childNodes'] as $item_node) {
              $item_name = $item_node['name'];
              $new_item_name = str_replace($submodel_prefer . ".", '', $item_name);
              $item_name_prefer = substr($item_name, 0, strpos($item_name, ']'));
...
function zipfile($filearray, $filename)
{
  ...
  $ow = 1;
  //$filename=iconv("utf-8","gb2312//IGNORE",$filename);
  if ($zip->open($filename, $ow ? ZIPARCHIVE::OVERWRITE : ZIPARCHIVE::CREATE) === TRUE) {
