...
class openTelnet():
    ...
    """
    def __init__(self):
        self.login_url  = 'http://192.168.0.1/login/Auth'
        self.telnet_url = 'http://192.168.0.1/goform/telnet'
    ...
    def login(self):
        ...
        res          = session.post(self.login_url, data=json.dumps(data), headers=header)
        self.cookies = 'bLanguage=en; _:USERNAME:_=' + res.cookies['_:USERNAME:_']
    ...
    def telnet_login(self):
        ...
        session = requests.Session()
        res     = session.get(self.telnet_url, headers=header)
        if 'load telnetd success' in res.content.decode('utf-8'):
            return True
        return False
...
class cpuInfo(Loggers):
    ...
    def __init__(self):
        ...
        if openTelnet().telnet_login():
            self.logger.info("开启DUT telnet成功")
        else:
            self.logger.info("开启DUT telnet失败")
            print("error")
            sys.exit()
        self.tn = Telnet()
